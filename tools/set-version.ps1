...
function Write-Utf8StagingFile {
    ...
    [byte[]]$bytes = New-Object byte[] ($preambleLength + $contentBytes.Length)
    if ($WithBom) {
        $bytes[0] = 0xEF
        $bytes[1] = 0xBB
        $bytes[2] = 0xBF
...
$resourcePath = Join-Path $projectRoot 'resources\version.rc'
$manifestPath = Join-Path $projectRoot 'src\App\app.manifest'
...
$cmake = Read-Utf8TextFile -Path $cmakePath
$resource = Read-Utf8TextFile -Path $resourcePath
$manifest = Read-Utf8TextFile -Path $manifestPath
$versionFile = Read-Utf8TextFile -Path $versionFilePath
...
$updatedCmake = Replace-ExactlyOnce `
    -Text $cmake.Text `
...
$updatedVersionFile = Replace-ExactlyOnce `
    -Text $versionFile.Text `
    -Pattern '\A([0-9]+\.[0-9]+\.[0-9]+)(\r?\n)?\z' `
...
$updates = @(
    ...
    [pscustomobject]@{
        Path = $cmakePath
        Original = $cmake.Text
        Updated = $updatedCmake
        HasUtf8Bom = [bool]$cmake.HasUtf8Bom
    },
    [pscustomobject]@{
        Path = $resourcePath
    ...
    [pscustomobject]@{
        ...
        Original = $versionFile.Text
        Updated = $updatedVersionFile
        # The online client intentionally accepts only ASCII release metadata.
        HasUtf8Bom = $false
    }
)
...
try {
    ...
    foreach ($update in $changedUpdates) {
        $verified = Read-Utf8TextFile -Path $update.Path
        if ($verified.Text -cne $update.Updated -or
            [bool]$verified.HasUtf8Bom -ne [bool]$update.HasUtf8Bom) {
            throw "Post-write version verification failed: $($update.Path)"
        }
    }
...
Write-Host "Updated FramePlayer version atomically: $Version ($fourPartVersion)"
