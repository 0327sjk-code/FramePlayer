...
constexpr std::wstring_view ProductExecutableName = L"FramePlayer.exe";
constexpr std::wstring_view UpdateRootDirectoryName = L"FramePlayer-Update";
...
bool VerifyInstalledCopy(
    ...
    if (!QueryFileSize(source, &sourceSize, error) ||
        !QueryFileSize(target, &targetSize, error) ||
    ...
    DWORD sourceHashError = ERROR_SUCCESS;
    DWORD targetHashError = ERROR_SUCCESS;
    const std::optional<Sha256Digest> sourceHash =
        ComputeFileSha256(source, &sourceHashError);
    const std::optional<Sha256Digest> targetHash =
        ComputeFileSha256(target, &targetHashError);
    if (!sourceHash.has_value() || !targetHash.has_value()) {
        if (error != nullptr) {
            *error = sourceHash.has_value()
                ? targetHashError
                : sourceHashError;
        }
        return false;
    }
    if (!Sha256Equals(*sourceHash, *targetHash)) {
        if (error != nullptr) {
            *error = ERROR_CRC;
        }
        return false;
