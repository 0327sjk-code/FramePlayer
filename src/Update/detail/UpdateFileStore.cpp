...
ExecutableDownloadResult AcquireImpl(
    ...
    if (ValidatePortableExecutable(
        ...
        const std::optional<Sha256Digest> cachedChecksum =
            ComputeFileSha256(*destination, &hashError);
        if (cachedChecksum.has_value() &&
            Sha256Equals(*cachedChecksum, expectedChecksum)) {
    ...
    const std::optional<Sha256Digest> actualChecksum =
        ComputeFileSha256(partialPath, &hashError);
    if (!actualChecksum.has_value()) {
        return {
            {}, 0, false,
            MakeFailure(
                UpdateErrorCode::FileSystem,
                L"Could not calculate the downloaded executable checksum.",
                hashError)};
    }
    if (!Sha256Equals(*actualChecksum, expectedChecksum)) {
        return {
            {}, 0, false,
            MakeFailure(
                UpdateErrorCode::InvalidChecksum,
                L"The downloaded executable SHA-256 checksum does not match the release manifest.")};
    }
    if (!::MoveFileExW(
            partialPath.c_str(),
            destination->c_str(),
