...
constexpr DWORD InstalledHealthStabilityGraceMs = 1'500;
...
[[nodiscard]] std::wstring BuildBootstrapCommandLine(
    ...
    const std::string checksumHex = Sha256ToHex(expectedChecksum);
    commandLine.reserve(
        downloadedExecutable.native().size() + targetExecutable.native().size() +
        processId.size() + checksumHex.size() + 112);
    commandLine.push_back(L'"');
    commandLine.append(downloadedExecutable.native());
    commandLine.append(L"\" ");
    commandLine.append(update_command_line::ApplyUpdateArgument);
    commandLine.push_back(L' ');
    commandLine.append(update_command_line::TargetExecutablePrefix);
    commandLine.push_back(L'"');
    commandLine.append(targetExecutable.native());
    commandLine.append(L"\" ");
    commandLine.append(update_command_line::ParentProcessIdPrefix);
    commandLine.append(processId);
    commandLine.push_back(L' ');
    commandLine.append(update_command_line::ExpectedChecksumPrefix);
    for (const char character : checksumHex) {
...
[[nodiscard]] std::wstring BuildInstalledCommandLine(
    ...
    commandLine.append(targetExecutable.native());
    commandLine.append(L"\" ");
    commandLine.append(update_command_line::CleanupUpdatePrefix);
    commandLine.push_back(L'"');
    commandLine.append(bootstrapExecutable.native());
    commandLine.append(L"\" ");
    commandLine.append(update_command_line::UpdateBootstrapProcessIdPrefix);
    commandLine.append(processId);
    commandLine.push_back(L' ');
    commandLine.append(update_command_line::UpdateHealthEventPrefix);
...
[[nodiscard]] bool CreateUniqueHealthEvent(
    ...
    for (std::uint32_t attempt = 0; attempt < MaximumAttempts; ++attempt) {
        std::wstring candidate = update_command_line::UpdateHealthEventNamePrefix;
...
InstalledHealthOutcome WaitForInstalledHealth(
    ...
    if (waitResult == WAIT_OBJECT_0 + 1) {
        const DWORD stabilityResult = ::WaitForSingleObject(
            installedProcess.ProcessHandle(),
            InstalledHealthStabilityGraceMs);
        if (stabilityResult == WAIT_TIMEOUT) {
            return InstalledHealthOutcome::Ready;
        }
        if (stabilityResult == WAIT_OBJECT_0) {
            return InstalledHealthOutcome::ProcessExited;
        }
        if (error != nullptr) {
            *error = ::GetLastError();
