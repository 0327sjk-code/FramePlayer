...
void UpdateCoordinator::SetStatusCallback(StatusCallback callback) noexcept {
    try {
        ...
        // A callback update must not destabilize the player process.
    }
}
...
void UpdateCoordinator::RunDownload(
    ...
    try {
        ...
        if (!result.verifiedChecksum.has_value()) {
            Finish(MakeTerminalFailure(
                currentVersion_,
                latestVersion,
                MakeCoordinatorFailure(
                    UpdateErrorCode::InvalidChecksum,
                    L"The downloaded update did not retain its verified checksum.")));
            return;
        }
        ...
        ready.verifiedChecksum = result.verifiedChecksum;
